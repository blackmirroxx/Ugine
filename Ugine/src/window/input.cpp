...
bool ugine::window::SDLInput::is_mouse_button_pressed(ugine::utils::mouse_button button) const noexcept {
    return SDL_BUTTON(ugine::utils::mouse_button_to_sdl_button(button)) == SDL_GetMouseState(nullptr, nullptr);
}

std::pair<float, float> ugine::window::SDLInput::get_mouse_position() const noexcept {
    int mouse_x = 0;
    int mouse_y = 0;
